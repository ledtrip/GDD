...
calculate_gdd_at_heading_2 <- function(data, t_lower, t_upper, t_base = 10) {
  data %>%
    mutate(
      GDD = case_when(
        PRISMMaxTempC <= t_lower ~ 0,                                   # No accumulation: both temps too low
        PRISMMinTempC >= t_upper ~ t_upper - t_base,                    # Fully saturated day
        # TRUE ~ ((pmin(PRISMMaxTempC, t_upper) + pmax(PRISMMinTempC, t_lower)) / 2 - t_base) # Seba: commented out and edited 
        TRUE ~ ((PRISMMaxTempC + PRISMMinTempC) / 2 - t_base)
      ),
      GDD = pmax(GDD, 0)
    ) %>%
    group_by(Location, Year, Variety) %>%
...
# Seba: Calculating GDD with edited function ####

# ✅ Step X: Export GDD values at heading for all threshold combos
all_gdd_records_2 <- map_dfr(1:nrow(grid), function(i) {
  row <- grid[i, ]
  gdd_data <- calculate_gdd_at_heading_2(df_Jul25_long, row$T_lower, row$T_upper)
...
# gdd_all <- read_csv("Outputs/GDD_AtHeading_AllThresholds.csv") %>% # Seba: commented out for effiiency
#   mutate(
#     threshold_label = paste0("T_low=", T_lower, ", T_up=", T_upper)
#   )

gdd_all_2 <- all_gdd_records_2 %>% # Seba: same output as passing through the csv file
  mutate(
    threshold_label = paste0("T_low=", T_lower, ", T_up=", T_upper)
  )
...
# Loop for both PNG and Pdf_Jul25
...
# 📂 Load GDD results
gdd_all_2 <- read_csv("Outputs/GDD_AtHeading_AllThresholds.csv")
...
for (i in seq_len(nrow(unique_thresholds))) {
  ...
  # Save PNG
  ggsave(
    filename = file.path(output_dir, paste0("gdd_all_2Varieties_T", t_low, "_", t_up, ".png")),
    plot = p, width = 8, height = 6
  )
}

# 📄 Combine all plots into a Pdf
pdf_Jul25("Outputs/gdd_all_2Varieties_Thresholds_Combined.pdf", width = 8, height = 6)
...
# gdd_all_2 <- read_csv("Outputs/GDD_AtHeading_AllThresholds.csv")  # Seba: commented out for efficiency. "all_gdd_records_2" is the same, "gdd_all_2" replaced hereon

# Calculate metrics 

## Seba: a) per variety ####

metrics_variety_2 <- all_gdd_records_2 %>%
...
# Best by RMSE
best_rmse_2 <- metrics_variety_2 %>%
  group_by(Variety) %>%
  slice_min(RMSE, n = 1) %>%
  ungroup()

# Best by R²
best_r2_2 <- metrics_variety_2 %>%
  group_by(Variety) %>%
  slice_max(R2, n = 1) %>%
  ungroup()

# Combined rank (RMSE low + R2 high)
best_combo_2 <- metrics_variety_2 %>%
  group_by(Variety) %>%
  mutate(
    rank_rmse = rank(RMSE),
...
metrics_all_combined_2 <- all_gdd_records_2 %>%
  filter(!is.na(cum_gdd), !is.na(DaysToHeading)) %>%
...
best_all_rmse_2 <- slice_min(metrics_all_combined_2, RMSE, n = 1)
best_all_r2_2   <- slice_max(metrics_all_combined_2, R2, n = 1)
best_all_combo_2 <- metrics_all_combined_2 %>%
  mutate(
    rank_rmse = rank(RMSE),
    rank_r2 = rank(desc(R2)),
    combo_score = rank_rmse + rank_r2
...
unique_combos_2 <- all_gdd_records_2 %>%
  distinct(Variety, T_lower, T_upper)
...
metrics_results_2 <- map_dfr(seq_len(nrow(unique_combos_2)), function(i) {
  pb$tick()
  row <- unique_combos[i, ]
  subset <- all_gdd_records_2 %>%
...
# Save output
write_csv(metrics_results_2, "Outputs/GDD_Metrics_FullGrid_2.csv")
...
# all_gdd_records_2 <- read_csv("Outputs/GDD_AtHeading_AllThresholds.csv") # Seba: commented out for efficiency. all_gdd_records_2 is the same, replaced hereon

# ✅ Calculate RMSE and R² per Variety and Threshold Combo
metrics_variety <- all_gdd_records_2 %>%
  filter(!is.na(cum_gdd), !is.na(DaysToHeading)) %>%
  group_by(Variety, T_lower, T_upper) %>%
  summarise(
...
metrics_combined <- all_gdd_records_2 %>%
  filter(!is.na(cum_gdd), !is.na(DaysToHeading)) %>%
  group_by(T_lower, T_upper) %>%
  summarise(
    RMSE = tryCatch(rmse(DaysToHeading, predict(lm(DaysToHeading ~ cum_gdd))),
